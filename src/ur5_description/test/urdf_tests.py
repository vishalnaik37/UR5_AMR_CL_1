...
panda_xacro_file_name = path.join(get_package_share_directory('ur5_description'), 'robots',
...
def test_load_with_robot_ip():
    urdf = xacro.process_file(panda_xacro_file_name,
                              mappings={'robot_ip': 'ur5_ip_address'}).toxml()
    assert urdf.find('ur5_ip_address') != -1
